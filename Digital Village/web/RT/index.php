...
$sql = "SELECT rt, rw FROM master_rt_rw WHERE nik = '$nik'";
...
$rt = $_SESSION['rt'];  // Mendapatkan RT dari session
$rw = $_SESSION['rw'];  // Mendapatkan RW dari session
// Query untuk menghitung jumlah baris dengan status 'Diajukan'
$query = "SELECT COUNT(*) AS total FROM `view_pengajuan_diajukan` WHERE status = 'Diajukan' AND rt = '$rt' AND rw = '$rw'";
...
$query1 = "SELECT COUNT(*) AS total FROM `view_pengajuan_diajukan` WHERE status = 'Disetujui RT' AND rt = '$rt' AND rw = '$rw'";
...
// Query untuk menghitung jumlah baris dengan status 'Ditolak'
$query2 = "SELECT COUNT(*) AS total FROM `view_pengajuan_diajukan` WHERE status = 'Ditolak' AND rt = '$rt' AND rw = '$rw'";
$result2 = $conn->query($query2);

if ($result2) {
    $row2 = $result2->fetch_assoc();
    $total2 = $row2['total']; // Ambil jumlah baris
} else {
    echo "Terjadi kesalahan: " . $conn->error;
}

// Query untuk mengambil data jenis pengajuan surat dari master_surat
$query3 = "SELECT * FROM master_surat";
        ...
        <div class="main p-3">
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="text">
                        <h1>Dashboard</h1>
                        <h6>Selamat Datang Ketua RT <?= $rt ?> RW <?= $rw ?></h6>
                    </div>
                    ...
                    <table class="table mt-3">
                        ...
                        <tbody>
                            <?php
                            // Menampilkan data jenis pengajuan surat dari master_surat
                            if ($result3->num_rows > 0) {
                                $no = 1;
                                while ($row3 = $result3->fetch_assoc()) {
                                    echo "<tr>";
                                    echo "<th scope='row'>" . $no++ . "</th>";
                                    echo "<td>" . $row3['nama_surat'] . "</td>";
                                    echo "</tr>";
                                }
                            } else {
